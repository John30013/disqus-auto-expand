...
(async () => {
  try {
    ...
    console.warn(
      "Get config from sync'd storage failed; using defaultConfig.",
      error
    );
  }
  // removeIf(!allowDebug)
  ...
  browser.runtime.sendMessage({ action: "setIcon", data: _config.isEnabled });
...
// Note: Chrome version has getCurrentConfig() here, but it's not needed in the
// Firefox version.
